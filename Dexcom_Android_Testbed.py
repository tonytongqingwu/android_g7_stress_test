...
class android_testbed(Common_Utilities):
    ...
    def logcat_fire(self):
        ...
        while True:
            ...
            pi = subprocess.Popen(order, shell=True, stdout=subprocess.PIPE,
                                  creationflags=0x00000200)  # creationflags=0x00000200 to ignore keyboard interrupt signal on logcat thread
            t = 0
            conn_count = 0
            prev_time = 0
            curr_time = 0
                ...
                try:
                    for i in iter(pi.stdout.readline, 'b'):
                        if i == b'' and self.logcat_restart_handle == True:
                            ...
                            file_filter.write(
                                "Starting Logcat Again, probably due to loose wired connection/loss of adb server")
                            file_filter.write("\n")
                            break
                        if i != b'':
                            file.write(str(i))
                        ...
                        if "TransmitterBLE: Connected" in str(i):
                            ...
                            if conn_count > 1:
                                tdelta = datetime.datetime.strptime(str(curr_time), FMT) - datetime.datetime.strptime(
                                    str(prev_time), FMT)
                                ...
                                if (comm_interval_gap) >= 320:
                                    ...
                                    file_filter.write(
                                        "Connection took more than 5 mins to establish between " + prev_time + " and " + curr_time)
                                    file_filter.write("\n")
                                    ...
                                    if pkts_lost >= 3:
                                        ...
                                        file_filter.write(
                                            "###Signal loss could be detected between " + prev_time + " and " + curr_time)
                                        file_filter.write("\n")
    ...
    def monitor_testbed_timer(self, end_time, current_process_pid):
        ...
        while True:
            if datetime.datetime.now() >= end_time:
                print(
                    "\033[91mTestbed Timer set by user is about to expire, time to collect HCI/snoop logs, post which Testbed application will exit\033[0m")
    ...
    def android_testbed_execute(self, current_process_pid):
        ...
        if DEXCOM_TIMER == True:
            ...
            self.monitor_testbed_timer_thread = Thread(target=self.monitor_testbed_timer,
                                                       args=(end_time, current_process_pid,))
...
def main_window():
    ...
    if var1.get() == 1:
        ...
        print(
            "\033[91mPause and Play Feature cannot be used here. Do not give keybaordinterrupt to pause the Testbed..!![0m")
...
def Test_Bed_Scenarios_Info(testbed_scenario):
    if testbed_scenario == "Scenario-1":
        messagebox.showinfo("Scenario_1",
        ...
        messagebox.showinfo("Scenario_6",
                            "List of Applications to be executed : \n\nLaunch_Operate Dexcom App\nLaunch Youtube\nLaunch Camera and capture photo\nLaunch YouTube Music\nLaunch FaceBook\nLaunch Youtube\nLaunch Maps\nLaunch Youtube\nLaunch Instagram\nLaunch Youtube\nLaunch Maps\nLaunch Youtube\nLaunch Instagram\nLaunch YouTubeMusic\nLaunch Maps\nLaunch Youtube\nLaunch FaceBook\nLaunch Youtube\nLaunch Camera and capture photo\nLaunch Youtube\nLaunch Maps\nLaunch Youtube\nLaunch Maps\nLaunch YouTubeMusic\nLaunch FaceBook\nLaunch Youtube\nLaunch Maps\nLaunch Youtube\nLaunch Instagram\nLaunch Youtube\nLaunch Maps\nLaunch Youtube\n\n [10 mins Default Timer for apps like Youtube, Music]")

    elif testbed_scenario == "Scenario-7":
        messagebox.showinfo("Scenario_7",
...
if __name__ == '__main__':
    ...
    duration_entry.configure(state="disabled")
    var1 = tk.IntVar()
    c1 = ttk.Checkbutton(tab2, text='Enable Test-Bed Timer', variable=var1, onvalue=1, offvalue=0, takefocus=False,
                         command=save_button)
    c1.place(x=180, y=60)
...
starting the Execution.\n { Dexcom G6 App(Warm-up Completed), YouTube, Flipkart, Amazon, Facebook, Chrome, YouTube Music etc. }',
                           font=('calibre', 10, 'bold'), foreground="dark red")
