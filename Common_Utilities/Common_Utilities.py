...
class Common_Utilities:
    ...
    def makeaCall(self):
        try:
            self.Press_Device_Home_Button()
            os.system("adb -s "+self.adb_id+" shell am start -a android.intent.action.CALL -d tel:'"+self.config_params.get("MobileNumber")+"'")
            sleep(8)
            try:
    ...
    def sendSMS(self):
        try:
            self.Press_Device_Home_Button()
            sleep(4)
            os.system("adb -s "+self.adb_id+" shell am start -a android.intent.action.SENDTO -d sms:"+self.mobile_no+"  --es  sms_body 'This is TestBed Generated message for testing purpose' --ez exit_on_sent true")
            sleep(4)
    ...
    def setBatteryPercentage(self):
        try:
            os.system("adb -s "+self.adb_id+" shell dumpsys battery set level '"+self.config_params.get("BatteryPercentage")+"'")
        except Exception as e:
            print(e)
    ...
    def Generate_HCILogs(self):
        ...
        p = subprocess.Popen(command, shell=True,
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=0x00000200)# creationflags=0x00000200 to ignore keyboard interrupt signal
        stdout, stderr = p.communicate()
        if (stdout!=""):
    ...
    def Detailed_Memory_Dexcom(self):
        try:    
            ...
            current_date_and_time = datetime.now()
            time=current_date_and_time.strftime("%Y-%m-%d-%H-%M-%S")
            os.system("adb -s "+self.adb_id+" shell dumpsys meminfo com.dexcom.g6 >" +path+ "\DetailedMemory_G6_"+time+".txt")
    ...
    def Current_Memory(self):
        try:
            ...
            current_date_and_time = datetime.now()
            time=current_date_and_time.strftime("%Y-%m-%d-%H-%M-%S")
            os.system("adb -s "+self.adb_id+" shell cat /proc/meminfo > " + path +"\CurrentMemoryUsage_"+time+".txt")    
    ...
    def List_of_AppsRunning(self):
        try:
            ...
            print("\033[95mList_of_AppsRunning File Generated @ Path : "+path+"/RunningAppsList_"+time+".txt \033[0m")
        except Exception as e:  
            print(e)
            for i in range(0,5):
                self.Press_Device_BackButton()
